...
% Pressures
p_cond_nom       = 2.79e6;               % [Pa]
%p_cond_nom       = 3.5e6;               % [Pa]

% Temperatures (cycle)
T_cond_sat_nom_C = 45;      % [°C] condensing saturation temp
SH_comp_nom      = 11.1;    % [K] superheat at compressor outlet (or inlet if same)
SC_cond_nom      = 8.3;     % [K] subcooling at condenser outlet

% Compressor outlet (superheated vapor)
T_comp_out_nom_C =  66.85;
%T_comp_out_nom_C = T_cond_sat_nom_C + SH_comp_nom;   % [°C] 

% Condenser outlet (subcooled liquid)
T_cond_out_nom_C = 41.85;
%T_cond_out_nom_C = T_cond_sat_nom_C - SH_comp_nom;   % [°C] 
